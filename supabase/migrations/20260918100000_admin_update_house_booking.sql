-- Requires the current bigint bookings/customer schema. No RLS policies change.
-- Only the trusted web server may invoke this function, after allow_booking checks.
do $$
begin
  if not exists(select 1 from information_schema.columns where table_schema='public'
    and table_name='bookings' and column_name='id' and data_type='bigint')
    or to_regclass('public.customers') is null then
    raise exception 'Modern bigint bookings/customer schema is required; reconcile legacy data before applying this migration';
  end if;
end;
$$;
...
begin
  ...
  if jsonb_typeof(p_values) is distinct from 'object'
    or not (p_values ?& array['check_in','check_out','customer_id','status','quantity','price_sell','deposit_amount','extra_charge','note'])
    or exists(select 1 from jsonb_object_keys(p_values) k where k not in
      ('check_in','check_out','customer_id','status','quantity','price_sell','deposit_amount','extra_charge','note')) then
  ...
  v_deposit := (p_values->>'deposit_amount')::numeric;
  ...
  if v_start is null or v_end is null or not isfinite(v_start) or not isfinite(v_end) or v_end <= v_start
    ...
    or v_deposit is null or v_deposit < 0 or v_deposit > 999999999.99 or v_deposit <> round(v_deposit,2)
  ...
  update public.bookings set
    ...
    deposit_amount = v_deposit, extra_charge = v_extra,

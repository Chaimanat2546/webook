-- The base quotation writer creates the row before the template wrapper can
-- attach its immutable layout snapshot. These fields therefore cannot be NOT
-- NULL at the base insert boundary; the wrapper validates and writes all four
-- template/layout fields together immediately afterwards.
alter table public.quotations
  alter column document_template_revision_snapshot drop not null,
  alter column document_layout_schema_version_snapshot drop not null,
  alter column document_layout_snapshot drop not null;

...
- Rendered the final PDFs with Poppler at 144 DPI and inspected the compact, multi-page, QR, and certification continuation pages. The one-page PDF retains the intended compact no-QR payment/settlement composition. The auto-PromptPay QR is visible and the QR-bearing sequential pages retain payment details and QR-height rows before the full-width settlement; two QR rows move certification/footer to a clean continuation page. The multi-page PDF has repeated item headings, legible Thai glyphs, intact rows, no clipped content, stable empty image slots, an intact settlement block, certification, and seller footer.

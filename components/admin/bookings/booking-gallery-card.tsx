...
export function BookingGalleryCard({ house, card, month, today, loading, error, onMonthChange, onRetry, onBookingSelect, onCreateSelect }: Props) {
  ...
  const disabled = house.is_active !== true;
  return <Card size="sm" className="booking-gallery-card min-w-0 gap-2">
    <CardHeader className="min-w-0 gap-1">
      ...
      <div className="flex items-center justify-center gap-2 pt-1">
        <Button type="button" variant="outline" size="icon-sm" aria-label={`เดือนก่อนหน้า ${house.title}`} disabled={disabled || !previousMonth}
        ...
        <Button type="button" variant="outline" size="icon-sm" aria-label={`เดือนถัดไป ${house.title}`} disabled={disabled || !nextMonth}
    ...
    <CardContent className="space-y-2" aria-busy={loading}>
      ...
      <Button type="button" size="sm" variant="outline" className="w-full" disabled={disabled} onClick={event => onCreateSelect(house.property_id, undefined, event.currentTarget)}>

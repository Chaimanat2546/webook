...
function ItemUnitControl({ index, item, onUpdate, labelled }: Pick<ItemProps, "index" | "item" | "onUpdate"> & { labelled?: boolean }) { return <TextInput field={`items.${index}.unit`} label={labelled ? "หน่วย" : undefined} onChange={(value) => onUpdate("unit", value)} size="compact" value={item.unit} />; }
  ...
  return <div className="space-y-4" data-dirty={isDirty} data-quotation-editor>
    ...
    <div data-workbench-metadata className="grid gap-6 lg:grid-cols-12">
      <section data-customer-section className="border-t border-foreground/35 pt-2 lg:col-span-7">
        ...
        <div data-customer-fields className="grid gap-3 sm:grid-cols-2"><div className="sm:col-span-2"><TextInput error={fieldErrors["customer.name"]} field="customer.name" label="ชื่อลูกค้า" onChange={(value) => updateCustomer("name", value)} size="name" value={payload.customer.name} /></div><div className="sm:col-span-2"><Field error={fieldErrors["customer.address"]} field="customer.address" label="ที่อยู่"><Textarea className={controlClassName("address")} data-field="customer.address" onChange={(event) => updateCustomer("address", event.target.value)} value={payload.customer.address} /></Field></div><TextInput error={fieldErrors["customer.taxId"]} field="customer.taxId" label="เลขผู้เสียภาษี" onChange={(value) => updateCustomer("taxId", value)} size="identifier" value={payload.customer.taxId} /><Field error={fieldErrors["customer.officeType"]} field="customer.officeType" label="สำนักงานลูกค้า"><select className={controlClassName("identifier", selectClassName)} data-field="customer.officeType" onChange={(event) => updateCustomerOfficeType(event.target.value === "branch" ? "branch" : "head_office")} value={payload.customer.officeType}><option value="head_office">สำนักงานใหญ่</option><option value="branch">สาขา</option></select></Field>{payload.customer.officeType === "branch" ? <TextInput error={fieldErrors["customer.branchNumber"]} field="customer.branchNumber" label="เลขสาขาลูกค้า" onChange={(value) => updateCustomer("branchNumber", value)} size="identifier" value={payload.customer.branchNumber} /> : null}<TextInput error={fieldErrors["customer.contactName"]} field="customer.contactName" label="ผู้ติดต่อ" onChange={(value) => updateCustomer("contactName", value)} size="person" value={payload.customer.contactName} /><TextInput error={fieldErrors["customer.phone"]} field="customer.phone" label="โทรศัพท์" onChange={(value) => updateCustomer("phone", value)} size="person" value={payload.customer.phone} /><TextInput error={fieldErrors["customer.email"]} field="customer.email" label="อีเมล" onChange={(value) => updateCustomer("email", value)} size="contact" value={payload.customer.email} /></div>
      </section>

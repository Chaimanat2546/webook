...
export interface QuotationEditorProps { documentNumber: string | null; initialPayload: QuotationPayload; printOnLoad?: boolean; publicToken: string | null; }
...
function DocumentMore({ deleteEnabled, isPending, onClose, onDelete, onPreview, onSave }: { deleteEnabled: boolean; isPending: boolean; onClose: () => void; onDelete: () => void; onPreview: () => void; onSave: () => void }) { return <DropdownMenu><DropdownMenuTrigger asChild><Button type="button" variant="outline"><MoreHorizontal aria-hidden="true" className="size-4" />เพิ่มเติม</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={onPreview}><Eye aria-hidden="true" className="size-4" />ดูตัวอย่าง</DropdownMenuItem><DropdownMenuItem disabled={isPending} onSelect={onSave}><Save aria-hidden="true" className="size-4" />บันทึก</DropdownMenuItem><DropdownMenuItem onSelect={onClose}><X aria-hidden="true" className="size-4" />ปิด</DropdownMenuItem>{deleteEnabled ? <DropdownMenuItem onSelect={onDelete} variant="destructive"><Trash2 aria-hidden="true" className="size-4" />ลบ</DropdownMenuItem> : null}</DropdownMenuContent></DropdownMenu>; }
...
export function QuotationEditor({ documentNumber: initialDocumentNumber, initialPayload, printOnLoad = false, publicToken: initialPublicToken }: QuotationEditorProps) {
  ...
  function save(close = false) { setFormError(""); startTransition(async () => { const result = await saveQuotationAction(payload); if (!result.ok) { setFieldErrors(result.fieldErrors); setFormError(result.formError); const firstField = Object.keys(result.fieldErrors)[0]; if (firstField) requestAnimationFrame(() => focusField(firstField)); return; } setLastSavedPayload(payload); setPayload((current) => ({ ...current, id: result.id })); setDocumentNumber(result.documentNumber); setPublicToken(result.publicToken); setFieldErrors({}); setIsDirty(false); if (close) router.push("/admin/quotations"); else if (!payload.id) router.replace(`/admin/quotations/${encodeURIComponent(result.id)}?saved=1`); else router.refresh(); }); }
  ...
  async function shareSaved() { if (!publicToken) return; try { await navigator.clipboard.writeText(`${window.location.origin}/q/${publicToken}`); toast.success("คัดลอกลิงก์สาธารณะแล้ว"); } catch { toast.error("ไม่สามารถคัดลอกลิงก์ได้"); } }
  ...
  return <div className="space-y-4" data-dirty={isDirty} data-quotation-editor>
    <header className="flex flex-wrap items-start justify-between gap-3 border-b border-foreground/25 pb-3" data-workbench-command-bar>
      ...
      <div className="flex items-center gap-2" data-header-actions>
        <Button onClick={closeEditor} type="button" variant="outline">ปิด</Button>
        <Button disabled={isPending} onClick={() => save()} type="button">{isPending ? "กำลังบันทึก" : "บันทึก"}</Button>
      </div>
    </header>
    ...
    <section className="flex flex-wrap items-center justify-between gap-3 border-b py-2" data-seller-strip><div className="flex min-w-0 items-center gap-3">{payload.seller.logoUrl && !logoUnavailable ? <img alt="โลโก้ผู้ขาย" className="max-h-12 max-w-24 object-contain" onError={() => setLogoUnavailable(true)} src={payload.seller.logoUrl} /> : <div className="grid h-10 w-16 place-items-center rounded border text-xs text-muted-foreground">โลโก้</div>}<div className="min-w-0"><p className="truncate font-medium">{payload.seller.name || "ยังไม่มีชื่อผู้ขาย"}</p><p className="truncate text-sm text-muted-foreground">{payload.seller.officeType === "branch" ? `สาขา ${payload.seller.branchNumber || "-"}` : "สำนักงานใหญ่"}{payload.seller.taxId ? ` · ${payload.seller.taxId}` : ""}</p></div><Button onClick={() => setSellerExpanded((open) => !open)} size="sm" type="button" variant="ghost">แก้ไขเฉพาะใบ</Button></div><div className="flex flex-wrap items-center gap-2" data-document-actions><Button disabled={!publicToken || isPending} onClick={shareSaved} size="sm" type="button" variant="outline"><Share2 aria-hidden="true" className="size-4" />แชร์</Button><Button disabled={!canPrint} onClick={printSaved} size="sm" type="button" variant="outline"><Printer aria-hidden="true" className="size-4" />พิมพ์</Button><Button disabled size="sm" title="ยังไม่รองรับใน MVP นี้" type="button" variant="outline"><Download aria-hidden="true" className="size-4" />ดาวน์โหลด</Button><DocumentMore deleteEnabled={Boolean(payload.id)} isPending={isPending} onClose={closeEditor} onDelete={() => setDeleteOpen(true)} onPreview={() => setPreviewOpen(true)} onSave={() => save()} /></div></section>

...
function SortablePaymentMethod<T extends QuotationPaymentMethod>({ banks, errors, index, method, mode, onPatch, onRemove }: { banks: BankOption[]; errors: Record<string, string>; index: number; method: T; mode: "master" | "quotation"; onPatch: (patch: Partial<T>) => void; onRemove: () => void }) {
  ...
  return <article className={cn("min-w-0 border-b py-4 last:border-b-0", isDragging && "opacity-60")} data-payment-method ref={ref}>
    <header className="mb-3 flex items-center gap-2"><Button aria-label={`ลากเพื่อจัดลำดับช่องทางชำระเงิน ${index + 1}`} ref={handleRef} size="icon-xs" type="button" variant="ghost"><GripVertical aria-hidden="true" /></Button><Field label="ประเภท"><select className="h-8 w-full rounded-md border bg-transparent px-2 text-sm" onChange={(event) => updateType(event.target.value as T["type"])} value={method.type}>{paymentTypes.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>{mode === "master" ? <label className="ml-auto flex shrink-0 items-center gap-2 text-xs text-muted-foreground"><Switch checked={"isDefault" in method && method.isDefault === true} onCheckedChange={(checked) => onPatch({ isDefault: checked } as unknown as Partial<T>)} size="sm" />เลือกอัตโนมัติในใบใหม่</label> : null}<Button aria-label={`ลบช่องทางชำระเงิน ${index + 1}`} className="ml-auto" onClick={onRemove} size="icon-xs" type="button" variant="ghost"><Trash2 aria-hidden="true" /></Button></header>
    {method.type === "bank_transfer" ? <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-4"><Field error={error("bankId")} label="ธนาคาร"><select className="h-8 w-full rounded-md border bg-transparent px-2 text-sm" onChange={(event) => selectBank(event.target.value)} value={editorState.bankSelectValue}><option value="OTHER">อื่น ๆ</option>{banks.filter((bank) => bank.code !== "OTHER").map((bank) => <option key={bank.id} value={bank.id}>{bank.name}</option>)}</select></Field><Field error={error("accountName")} label="ชื่อบัญชี"><Input className="h-8" onChange={(event) => update("accountName", event.target.value as T["accountName"])} value={method.accountName} /></Field><Field error={error("accountNumber")} label="เลขที่บัญชี"><Input className="h-8" inputMode="numeric" onChange={(event) => update("accountNumber", event.target.value as T["accountNumber"])} value={method.accountNumber} /></Field><Field label="QR โอนเงิน"><select className="h-8 w-full rounded-md border bg-transparent px-2 text-sm" onChange={(event) => update("qrMode", event.target.value as T["qrMode"])} value={method.qrMode}><option value="none">ไม่ใช้</option><option value="upload">อัปโหลด QR</option></select></Field>{editorState.hasCustomBankFields ? <><Field error={error("customBankName")} label="ชื่อธนาคารอื่น"><Input className="h-8" onChange={(event) => update("customBankName", event.target.value as T["customBankName"])} value={method.customBankName} /></Field><PaymentImageInput disabled={uploading} label="โลโก้ธนาคารอื่น" onChange={(file) => upload("customBankLogoUrl", file)} /></> : null}</div> : null}
    ...
    {editorState.showQrUpload ? <div className="mt-3"><PaymentImageInput disabled={uploading} label="รูป QR" onChange={(file) => upload("qrImageUrl", file)} />{error("qrImageUrl") ? <p className="text-xs text-destructive">{error("qrImageUrl")}</p> : null}</div> : null}

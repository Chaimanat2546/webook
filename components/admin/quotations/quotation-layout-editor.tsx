...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, History, Redo2, RotateCcw, Undo2 } from "lucide-react";
...
function LayoutBlock({ block, canMove, config, isPending, onMove, onSelect, selected, template }: { block: QuotationLayoutBlock; canMove: (id: QuotationLayoutBlockId, direction: LayoutMoveDirection) => boolean; config: QuotationLayoutConfig; isPending: boolean; onMove: (id: QuotationLayoutBlockId, direction: LayoutMoveDirection) => void; onSelect: (id: QuotationLayoutBlockId) => void; selected: boolean; template: QuotationTemplate }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]" data-quotation-layout-editor>
    <Card className="overflow-hidden">
      ...
      <CardContent className="bg-muted/20 p-4 sm:p-6"><div className={cn("mx-auto grid w-full max-w-[210mm] gap-5 rounded-sm p-5 shadow-md ring-1 ring-border sm:p-7", TEMPLATE_CANVAS[template].frame)} data-layout-a4-canvas data-layout-template={template}>
        ...
        {[...movableZones, "footer" as const].map((zone, zoneIndex) => { const blocks = blocksInZone(draft, zone); if (!blocks.length) return null; return <section className="grid gap-2" data-layout-zone={zone} key={zone}><div className="flex items-center gap-2"><span className={cn("h-px flex-1", TEMPLATE_CANVAS[template].zoneLine)} /><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{ZONE_LABELS[zone]}</p>{zone !== "footer" ? <div className="flex gap-1" data-layout-section-controls><Button aria-label={`ย้าย ${ZONE_LABELS[zone]} ขึ้น`} disabled={isPending || zoneIndex === 0} onClick={() => moveDocumentSection(zone, "up")} size="icon-xs" type="button" variant="ghost"><ArrowUp aria-hidden="true" /></Button><Button aria-label={`ย้าย ${ZONE_LABELS[zone]} ลง`} disabled={isPending || zoneIndex === movableZones.length - 1} onClick={() => moveDocumentSection(zone, "down")} size="icon-xs" type="button" variant="ghost"><ArrowDown aria-hidden="true" /></Button></div> : null}<span className={cn("h-px flex-1", TEMPLATE_CANVAS[template].zoneLine)} /></div><div className="grid grid-cols-12 gap-2.5">{blocks.map((block) => <LayoutBlock block={block} canMove={canMoveFromLayout} config={draft} isPending={isPending} key={block.id} onMove={moveFromLayout} onSelect={setSelectedId} selected={selected?.id === block.id} template={template} />)}</div></section>; })}
      </div></CardContent>
    </Card>

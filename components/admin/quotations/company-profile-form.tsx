...
export function CompanyProfileForm({ initialSeller }: { initialSeller: SellerSnapshot }) {
  ...
  return <form className="grid gap-4" onSubmit={submit}>
    <Card><CardHeader><CardTitle>ข้อมูลจดทะเบียน</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="grid gap-2"><Label htmlFor="officeType">ประเภทสำนักงาน</Label><select aria-describedby={fieldErrors.officeType ? "officeType-error" : undefined} aria-invalid={Boolean(fieldErrors.officeType)} className="h-9 rounded-md border bg-transparent px-3 text-sm" defaultValue={officeType} id="officeType" name="officeType" onChange={(event) => setOfficeType(event.target.value === "branch" ? "branch" : "head_office")}><option value="head_office">สำนักงานใหญ่</option><option value="branch">สาขา</option></select>{fieldErrors.officeType ? <p className="text-sm text-destructive" id="officeType-error">{fieldErrors.officeType}</p> : null}</div>
    ...
    <Card><CardHeader><CardTitle>ที่อยู่</CardTitle></CardHeader><CardContent><div className="grid gap-2"><Label htmlFor="address">ที่อยู่</Label><Textarea aria-describedby={fieldErrors.address ? "address-error" : undefined} aria-invalid={Boolean(fieldErrors.address)} defaultValue={initialSeller.address} id="address" name="address" required />{fieldErrors.address ? <p className="text-sm text-destructive" id="address-error">{fieldErrors.address}</p> : null}</div></CardContent></Card>
    <Card><CardHeader><CardTitle>ช่องทางติดต่อบริษัท</CardTitle></CardHeader><CardContent className="grid gap-4 lg:grid-cols-3"><Field error={fieldErrors.phone} label="เบอร์โทรศัพท์" name="phone" value={initialSeller.phone} /><Field error={fieldErrors.email} label="อีเมล" name="email" type="email" value={initialSeller.email} /><Field error={fieldErrors.website} label="เว็บไซต์" name="website" type="text" value={initialSeller.website} /></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle>โลโก้ผู้ขาย</CardTitle></CardHeader><CardContent className="grid gap-3">{displayedLogoUrl && !logoUnavailable ? <picture><img alt="โลโก้ผู้ขาย" className="max-h-32 max-w-48 object-contain" onError={() => setLogoUnavailable(true)} src={displayedLogoUrl} /></picture> : <p className="text-sm text-muted-foreground">ไม่สามารถแสดงโลโก้</p>}<div className="grid gap-2"><Label htmlFor="logo">เลือกโลโก้ใหม่</Label><Input accept="image/png,image/jpeg,image/webp" aria-describedby={logoError ? "logo-error" : undefined} aria-invalid={Boolean(logoError)} id="logo" name="logo" onChange={handleLogoChange} disabled={disabled} type="file" />{logoError ? <p className="text-sm text-destructive" id="logo-error">{logoError}</p> : null}<p className="text-sm text-muted-foreground">รองรับ PNG, JPEG หรือ WebP ขนาดไม่เกิน 10 MB</p></div></CardContent></Card>
...
function Field({ error, label, name, required, type = "text", value }: { error?: string; label: string; name: string; required?: boolean; type?: string; value: string }) {
  ...
  return <div className="grid gap-2"><Label htmlFor={name}>{label}</Label><Input aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} defaultValue={value} id={name} name={name} required={required} type={type} />{error ? <p className="text-sm text-destructive" id={errorId}>{error}</p> : null}</div>;
}

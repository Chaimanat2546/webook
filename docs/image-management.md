...
- Saving cover selection opens a confirmation dialog that lists selected images and allows final reordering before persistence.
...
- Public villa pages may read `public.images` with the anon role for display only. Public insert, update, and delete are not allowed.

...
- AWS/S3-backed images เป็น legacy/delete-only: อนุญาตเฉพาะการลบไฟล์จริงหลัง confirmation ห้าม upload, replace, หรือแก้ไฟล์จริงบน AWS/S3
- Database metadata และ physical file operations ต้องแยกกันชัดเจน service ต้อง route operation ตาม provider/source ที่เชื่อถือได้ และต้อง fail closed เมื่อ provider ไม่รองรับ operation นั้น
